...
class PlayState extends MusicBeatState
{
	...
	public var marblesSky:FlxSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'marbles':
				var skyTex:String = "marblesSky";
				if(SONG.song.toLowerCase() == "greyung")
				{
					skyTex = "greyungSky";
				}
				
				marblesSky = new FlxSprite().loadGraphic(Paths.image(skyTex));
				marblesSky.screenCenter();
				marblesSky.scrollFactor.set();
				marblesSky.antialiasing = true;
				add(marblesSky);

				var bg = new FlxSprite().loadGraphic(Paths.image('marblesBg'));
				bg.screenCenter();
				bg.x += 35;
				bg.y -= 10;
				add(bg);
		...
		if(dad.curCharacter.startsWith('mr-cuts')) {
		...
		timeTxt.setFormat(Paths.font("RAVIE.TTF"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		var songNameTxt = new FlxText(8, 720 - (1 * 20) - 8, FlxG.width + 1, SONG.song.replace('-', ' '), 20);
		songNameTxt.setFormat(Paths.font("RAVIE.TTF"), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		songNameTxt.scrollFactor.set();
		songNameTxt.borderSize = 1.25;
		songNameTxt.visible = !ClientPrefs.hideHud;
		add(songNameTxt);

		scoreTxt = new FlxText(FlxG.width - 8 - (29 * 20), 720 - (3 * 20) - 8, 29 * 20, "Score: 100000\nMisses: 000\nRating: Super! (100%) - Clear", 20);
		scoreTxt.setFormat(Paths.font("RAVIE.TTF"), 20, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("RAVIE.TTF"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function startCharacterPos(char:Character, ?gfCheck:Bool = false) {
		if(gfCheck && char.curCharacter.startsWith('mr-cuts')) { //IF DAD IS GIRLFRIEND, HE GOES TO HER POSITION
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Hey!':
				...
				if(value != 0) {
					if(dad.curCharacter.startsWith('mr-cuts')) { //Tutorial GF is actually Dad! The GF is an imposter!! ding ding ding ding ding ding ding, dindinding, end my suffering
				...
				switch(charType) {
					...
					case 1:
						if(dad.curCharacter != value2) {
							...
							var wasGf:Bool = dad.curCharacter.startsWith('mr-cuts');
	...
	function moveCameraSection():Void
	{
		...
		if (gf != null && SONG.notes[curSection].spikeCutsState == 1 && SONG.notes[curSection].mustHitSection)
		{
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				...
				if(note.gfNote || SONG.notes[curSection].spikeCutsState == 1)
				{
					if(gf != null)
					{
				...
				else if(SONG.notes[curSection].spikeCutsState == 2)
				{
					if(gf != null)
					{
						gf.playAnim(animToPlay + note.animSuffix, true);
	...
	override function sectionHit()
	{
		...
		if (SONG.notes[curSection] != null)
		{
			...
			setOnLuas('spikeCutsState', SONG.notes[curSection].spikeCutsState);
		}
